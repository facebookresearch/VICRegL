...
def parse_args():
    parser = argparse.ArgumentParser(
        "Submitit for VICRegL", parents=[main_vicregl.get_arguments()]
    )
    parser.add_argument(
    ...
    parser.add_argument("--job_name", default="vicregl", type=str, help="Job name")
    parser.add_argument(
...
def get_shared_folder() -> Path:
    ...
    if Path("/checkpoint/").is_dir():
        p = Path(f"/checkpoint/{user}/vicregl/experiments")
...
class Trainer(object):
    ...
    def _setup_gpu_args(self):
        ...
        self.args.exp_dir = Path(
            str(self.args.exp_dir).replace("%j", str(job_env.job_id))
        )
